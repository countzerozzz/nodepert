...
def linesearchfunc(randkey, params, start, stop, num, meta_data):
    ...
    learning_rates = np.logspace(
        start, stop, num, endpoint=True, base=10, dtype=np.float32
    )
    forward, step_type, n_hl, update_rule, hl_size, network_acc = meta_data
    df = pd.DataFrame()
    ...
    for lr in learning_rates:
        ...
        if step_type == "single-step":
            ...
            for x, y in data.get_data_batches(batchsize=100, split="test[:100%]"):
    ...
    df["step_type"], df["n_hl"], df["trajectory"], df["hl_size"], df["network_acc"] = (
        step_type,
        n_hl,
        update_rule,
        hl_size,
