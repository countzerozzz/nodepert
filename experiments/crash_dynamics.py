...
for epoch in range(0, num_epochs):
    ...
    for x, y in data.get_data_batches(
...
if crash:
    ...
    grad_norms_df = pd.DataFrame(
        columns=["gnorm_w" + str(i) for i in np.arange(1, len(layer_sizes))]
    )
    ...
    graddiff_norms_df = pd.DataFrame(
        columns=["gdiff_norm_w" + str(i) for i in np.arange(1, len(layer_sizes))]
    )
    ...
    sign_symmetry_df = pd.DataFrame(
        columns=["ss_w" + str(i) for i in np.arange(1, len(layer_sizes))]
    )
    ...
    grad_angles_df = pd.DataFrame(
        columns=["gangle_w" + str(i) for i in np.arange(1, len(layer_sizes))]
    )
    grad_angles_df["update_rule"], grad_angles_df["epoch"] = "", ""
    # dataframe to store the norm of the neural network weights
    w_norms_df = pd.DataFrame(
        columns=["norm_w" + str(i) for i in np.arange(1, len(layer_sizes))]
    )
    ...
    w_var_df = pd.DataFrame(
        columns=["var_w" + str(i) for i in np.arange(1, len(layer_sizes))]
    )
    w_var_df["update_rule"], w_var_df["epoch"] = "", ""
    # dataframe to store the activity of neurons at every layer
    n_activity_df = pd.DataFrame(
        columns=["activity_l" + str(i) for i in np.arange(1, len(layer_sizes))]
    )
    ...
    for ii in range(stored_epoch, stored_epoch + 5):
        ...
        for batch_id in range(num_batches):
            x, y = (
                xl[batch_id * batchsize : (batch_id + 1) * batchsize],
                yl[batch_id * batchsize : (batch_id + 1) * batchsize],
            )
            randkey, _ = random.split(randkey)
            ...
            if batch_id % interval == 0:
                test_acc.append(
                    train.compute_metrics(params_new, forward, data, split_percent)[1]
                )
                _, truegrad, _ = optim.sgdupdate(
                    xl, yl, params_new, randkey, optimstate
                )

                epoch = round(ii + (batch_id + 1) / num_batches, 3)

                if flags[0]:
                    grad_norms_df = grad_norms_df.append(
                        grad_dynamics.grad_norms(
                            npgrad, sgdgrad, truegrad, layer_sizes, epoch
                        )
                    )

                if flags[1]:
                    graddiff_norms_df = graddiff_norms_df.append(
                        grad_dynamics.graddiff_norms(
                            npgrad, sgdgrad, truegrad, layer_sizes, epoch
                        )
                    )

                if flags[2]:
                    sign_symmetry_df = sign_symmetry_df.append(
                        grad_dynamics.sign_symmetry(
                            npgrad, sgdgrad, truegrad, layer_sizes, epoch
                        )
                    )

                if flags[3]:
                    grad_angles_df = grad_angles_df.append(
                        grad_dynamics.grad_angles(
                            npgrad, sgdgrad, truegrad, layer_sizes, epoch
                        )
                    )

                if flags[4]:
                    w_norms_df = w_norms_df.append(
                        grad_dynamics.w_norms(params_new, layer_sizes, epoch)
                    )

                if flags[5]:
                    w_var_df = w_var_df.append(
                        grad_dynamics.w_vars(params_new, layer_sizes, epoch)
                    )

                if flags[6]:
                    n_activity_df = n_activity_df.append(
                        grad_dynamics.compute_layer_activity(
                            x, y, params_new, layer_sizes, epoch
                        )
                    )
...
graddiff_norms_df["test_acc"], graddiff_norms_df["jobid"] = (
    np.repeat(test_acc, 2),
    jobid,
)
...
if log_expdata:
    ...
    if flags[1]:
        graddiff_norms_df.to_csv(
            path + "graddiff_norms.csv", mode="a", header=use_header
        )
