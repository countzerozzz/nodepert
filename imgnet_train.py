...
def compute_metrics(params, forward, data, split_percent="[:100%]"):
    ...
    train_acc = []
    for batch_dict in data.get_rawdata_batches(batchsize=100, split="train"):
        x, y = batch_dict["image"], batch_dict["label"]
    ...
    test_acc = []
    for batch_dict in data.get_rawdata_batches(batchsize=100, split="validation"):
        x, y = batch_dict["image"], batch_dict["label"]
        x, y = data.prepare_data(x, y)
        h, a = forward(x, params)
...
def train(params, forward, data, config, optimizer, optimstate, randkey, verbose=True):
    print("this is the imagenet trainer!")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_dict in data.get_rawdata_batches(
