...
elif(network == 'conv-base'):
    num_layers = 3
    convchannels = 32
    #* Keras default values of initializer, [kernel: glorot, bias: zeros]
    model.add(tf.keras.layers.Conv2D(convchannels, (3, 3), strides=1, activation=actfunc, padding='same', input_shape=(32, 32, 3), kernel_initializer=initializer, bias_initializer='zeros'))
    model.add(tf.keras.layers.Conv2D(convchannels, (3, 3), strides=2, activation=actfunc, padding='same', kernel_initializer=initializer))
    model.add(tf.keras.layers.Conv2D(convchannels, (3, 3), strides=1, activation=actfunc, padding='same', kernel_initializer=initializer))
...
elif (network == 'All-CNN-A_on_steroids'):
    num_layers = 7
    initializer = tf.keras.initializers.GlorotNormal()
    model.add(tf.keras.layers.Conv2D(96, (5, 5), strides=1, activation=actfunc, padding='same', input_shape=(32, 32, 3), kernel_initializer=initializer, bias_initializer='zeros'))
...
if(log_expdata):
    ...
    if(not os.path.exists(path + network.lower() + '-TF.csv')):
        use_header = True
    
    df.to_csv(path + network.lower() + '-TF.csv', mode='a', header=use_header)
